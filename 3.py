...
starttime = time.time()
img1 = cv2.imread('./NO2.jpg') #query
img2 = cv2.imread('./NO1.jpg') #train
imageA = cv2.resize(img1,(0,0),fx=0.2,fy=0.2)
imageB = cv2.resize(img2,(0,0),fx=0.2,fy=0.2)
surf=cv2.SIFT_create()#可以改为SIFT
#sift = cv2.SIFT_create()
kp1,descrip1 = cv2.sift.detectAndCompute(imageA,None)
kp2,descrip2 = cv2.sift.detectAndCompute(imageB,None)
#创建字典
